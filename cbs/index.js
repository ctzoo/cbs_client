...
const { ERROR, VALIDATE_COMPLETED, BEGIN_PROC_WORKSHEET, ONE_RECORD_COMPLETED, WORKSHEET_COMPLETED } = require('../src/consts')
...
module.exports = async function(reqData /* request byte data */, ad /* action dispach */, send) {
  ...
  for (let j = 0; j < pkgs.length; j++) {
    ...
    for (let i = 0; i < pkgs[j][1].length; i++) {
      ...
      const htmls = gh(reqData, res)
      ret = ret.concat(htmls)
